...
import uk.gov.hmrc.incometaxsubscriptioneligibilityfrontend.views.html.principal.injected.{CovidCheck, Overview}
...
@Singleton
class Covid19ClaimCheckController @Inject()(covidCheck: CovidCheck, overview: Overview, auditService: AuditingService, mcc: MessagesControllerComponents)
  ...
  def show: Action[AnyContent] = Action.async {
    implicit request =>
      if (isEnabled(RemoveCovidPages)) {
        ...
        Future.successful(
          Ok(covidCheck(covid19ClaimCheckForm, routes.Covid19ClaimCheckController.submit()))
        )
      }
  }
  ...
  def submit: Action[AnyContent] = Action.async {
    implicit request =>
      covid19ClaimCheckForm.bindFromRequest.fold(
        formWithErrors => Future.successful(BadRequest(covidCheck(formWithErrors, routes.Covid19ClaimCheckController.submit()))), {
          case Yes =>
