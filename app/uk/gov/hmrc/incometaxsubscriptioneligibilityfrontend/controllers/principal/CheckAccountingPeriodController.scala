...
import uk.gov.hmrc.incometaxsubscriptioneligibilityfrontend.forms.CheckAccountingPeriodForm._
...
@Singleton
class CheckAccountingPeriodController @Inject()(mcc: MessagesControllerComponents)
                                               (implicit appConfig: AppConfig) extends FrontendController(mcc) with I18nSupport {

  def show: Action[AnyContent] = Action.async {
    implicit request =>
      Future.successful(
        Ok(accounting_period_check(checkAccountingPeriodForm, routes.CheckAccountingPeriodController.submit()))
      )
  }

  def submit(): Action[AnyContent] = Action.async {
    implicit request =>
      checkAccountingPeriodForm.bindFromRequest.fold(
