...
import uk.gov.hmrc.incometaxsubscriptioneligibilityfrontend.assets.MessageLookup.{suffix,Common, HaveAnyOtherIncome => messages}

class HaveAnyOtherIncomeControllerISpec extends ComponentSpecBase with ViewSpec {

  "GET /other-income" should {
    ...
    "return OK" in {
      ...
      "have a view with the correct values displayed in the form" in {
        ...
        labels.get(0).text() mustBe Common.yes

        radios.get(1).attr("id") mustBe "choice-No"
        labels.get(1).text() mustBe Common.no
